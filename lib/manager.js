...
Manager.prototype.onDispatch = function (rooms, packet, volatile, exceptions) {
  var ids = {};
  ...
  for (var j = 0, rl = rooms.length; j < rl; j++) {
    var room = rooms[j];
    ...
    if (this.rooms[room]) {
      for (var i = 0, l = this.rooms[room].length; i < l; i++) {
        ...
        if (!ids[id] && !~exceptions.indexOf(id)) {
